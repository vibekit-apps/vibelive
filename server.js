...
const USERS_FILE = path.join(DATA_DIR, 'accounts-v3.json');
...
const hasFullAccess = user => Boolean(user);
...
const publicUser = user => ({id:user.id, username:user.username, owner:isOwnerUsername(user.username), creator:isCreatorUsername(user.username), fullAccess:true, trialEndsAt:null, price:'Free'});
